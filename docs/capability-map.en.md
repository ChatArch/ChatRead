...
Use this page to check which first-class capabilities `ChatRead` currently owns, which ones are verified, and what remains out of scope for this package.
...
- **CLI Entry**
    ...
    `chatread --help`, `chatread --version`, `chatread --tree`, and `chatread --tree-brief` are the default verification entry points.
