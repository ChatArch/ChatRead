...
This page is the compact capability map for the `ChatRead` CLI. Use it to review which commands are first-class entries and which are still boundary or planned slots. After scaffolding, update it with the real command tree; do not present unimplemented commands as available operations.
...
```text
chatread                  # ChatRead command-line entry
...
```text
chatread --help           # Verify the command is installed and inspect the current command tree
chatread --version        # Verify the installed version
chatread --tree           # Read back the CLI contract with parameter signatures
chatread --tree-brief     # Read back command nodes and descriptions only
```
...
```text
chatread <group>          # Command group named after real package capability
